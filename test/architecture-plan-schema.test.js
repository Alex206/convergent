...
test('invalid explicit architecture significance is rejected while omission remains backward compatible', () => {
  const invalid = normalizePlan({ summary: 'one task', tasks: [{ id: 't1', title: 'Change helper', description: 'Change helper.', acceptanceCriteria: ['Works'], route: 'standard', risk: 'low', architectureSignificance: 'huge', routingReason: 'local' }] });
  assert.match(validatePlan(invalid), /architectureSignificance/);
  const legacy = normalizePlan({ summary: 'one task', tasks: [{ id: 't1', title: 'Change helper', description: 'Change helper.', acceptanceCriteria: ['Works'], route: 'standard', risk: 'low', routingReason: 'local' }] });
