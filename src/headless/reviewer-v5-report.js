...
const CASE_DEFECTS = Object.freeze({
  'v5-s19-dispatch-multidefect': [
    'tenant_idempotency_isolation',
    'stale_lease_after_reclaim',
    'terminal_queue_entry_not_claimable',
  ],
  'v5-s19-dispatch-clean': [],
  'v5-h22-budget-regression': ['accepted_report_order_invariance'],
  'v5-h22-budget-fixed': [],
});
...
function markdown(runs, summary) {
  const lines = [
    '# Reviewer V5 Cross-File + Historical Benchmark', '',
    '> Control-snapshot off-target findings require manual review; only seeded-defect presence/absence is automated.', '',
...
function main(argv = process.argv.slice(2)) {
  ...
  const jsonOutput = path.resolve(argv[1] ?? path.join(root, 'reviewer-v5-report.json'));
  const mdOutput = path.resolve(argv[2] ?? path.join(root, 'reviewer-v5-report.md'));
