...
Treat deterministic stable topological ordering operationally: at each emission step, all dependencies of the chosen task must already have been emitted, and the chosen task must be the earliest in original input order among the tasks that are currently dependency-ready. Do not impose a global pairwise input-order rule on arbitrary tasks when one was not ready at the same decision point; such a rule can contradict valid dependency constraints.

Derive graph witnesses from the task contract. For stable ordering, do not rely only on a simple chain or an all-independent list: include a mixed partial-order graph where dependencies change the ready set while unrelated tasks also compete for position. Use the reported ready_before / earliest_ready_input / chosen transition rows to reason about the observed behavior directly rather than mentally simulating the graph. Also exercise a genuine cycle when cycle handling is part of the task contract.
