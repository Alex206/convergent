...
async function inspectModels({ outputFile, coordinator = 'strong', reviewer = 'strong' } = {}, dependencies = {}) {
  ...
  try {
    ...
    const resolution = resolveHeadlessRoleModels({ coordinator, reviewer }, available);
    const report = {
      generatedAt: new Date().toISOString(),
      runtimeTransport: runtime.transport,
      selectors: { coordinator, reviewer },
    ...
    console.log(`Copilot model preflight: ${report.availableCount} available model(s); coordinator=${report.resolved.coordinator.id}; reviewer=${report.resolved.reviewer.id}; issues=${report.issues.length}.`);
    return report;
  } finally {
