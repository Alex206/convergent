...
4. Give every task explicit acceptance criteria and classify EACH task with route, risk, architectureSignificance, and routingReason:
   ...
   - high_risk: security/auth, concurrency, data/schema migrations, destructive behavior, production/release infrastructure, public API compatibility, or other changes where mistakes have high impact.
5. Risk must be low, medium, or high. Be conservative: uncertainty itself can raise risk. Risk describes failure impact/assurance need, not structural scope.
6. architectureSignificance must be low, medium, or high and is independent from risk:
   ...
   - high: a new/reworked subsystem, runtime/backend/provider/plugin/service/persistence/IPC boundary, interface redesign, or cross-cutting ownership change where architectural guidance can prevent structural debt.
   High architecture significance does NOT automatically mean high risk, and high risk does NOT automatically mean high architecture significance. Do not mark high merely because a diff may be large.
...
8. Call report_plan exactly once with the final classified plan. Do not edit files.
...
- Batch related edits into as few file-tool calls as practical. For tests, builds, long-running commands, subprocess-producing commands, or any command whose exact completion matters, use run_command so Convergent owns timeout/cancellation/process-tree evidence. Keep the built-in shell for short bounded inspection commands only. Never background a run_command command.
