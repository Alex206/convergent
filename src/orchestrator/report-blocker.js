...
function explicitBlockerEvidence(report = {}) {
  ...
  if (summary && !deniesBlockerLanguage(summary) && /\b(?:explicitly\s+)?blocked\b/i.test(summary)) {
    return summary;
  }
  for (const raw of report.checks ?? []) {
    const check = text(raw);
    if (!check || deniesBlockerLanguage(check)) continue;
    if (/\bBLOCKED\s*:/i.test(check) || /\b(?:required|external)\s+(?:validation|check)\b[^.]{0,120}\b(?:blocked|unavailable|not configured)\b/i.test(check)) {
      return check;
    }
  }
  return null;
