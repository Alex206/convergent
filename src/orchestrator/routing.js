...
function normalizeTaskRoute(task, routingMode = 'adaptive') {
  ...
  if (architecture === 'high' && route !== 'read_only') {
    reasons.push('architecturally significant change requires a bounded read-only software-architect assessment');
  }
...
function routePolicy(route, risk = 'medium', architecture = 'low') {
  ...
  switch (route) {
    ...
    case 'high_risk':
      return {
        description: `${architect ? 'Read-only architecture assessment, then ' : ''}full A/B convergence plus persistent strong review with higher reasoning effort.`,
        workerMode: 'converge',
        strongReview: true,
        architect,
        efforts: { architect: 'medium', workerA: 'medium', workerB: 'medium', reviewer: 'high' },
      };
    case 'standard':
    default:
      return {
        description: `${architect ? 'Read-only architecture assessment, then ' : ''}full A/B convergence plus persistent strong review.`,
        workerMode: 'converge',
        strongReview: true,
        architect,
        efforts: {
