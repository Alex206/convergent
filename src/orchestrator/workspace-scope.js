...
function benchmarkReviewContractPrompt(env = process.env) {
  ...
  return [
    ...
    'Before returning CLEAN, derive a compact acceptance matrix from every explicit task requirement and verify each criterion against the current implementation. Include type/shape constraints, negative cases, ordering semantics, diagnostics, compatibility, and other boundary behavior that the task text makes material.',
    'For each criterion, distinguish implementation evidence from test evidence. The existence of a test is not proof that the test exercises the requirement; challenge fixtures and assertions for the boundary case they are meant to cover.',
    'For semantic or algorithmic criteria, do not accept an algorithm name, implementation comment, or plausible-looking example as proof. Construct at least one minimal discriminating counterexample that would fail a common near-miss implementation. For ordering or scheduling invariants, explicitly consider a case where a newly eligible item competes with an already eligible independent item and verify the required relative order.',
    ...
    'If a remediation changed algorithmic behavior or fixed a correctness finding, that prior finding is a concrete reason for independent validation: before CLEAN, run one focused non-polluting probe or micro-test that exercises the discriminating boundary case instead of relying only on worker-reported tests.',
    ...
    'CLEAN is allowed only after every explicit acceptance criterion has implementation evidence and adequate validation evidence, or a concrete reason why separate validation is unnecessary.',
  ].join('\n');
}
