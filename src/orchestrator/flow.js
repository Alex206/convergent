...
function coordinatorFlowInstructions(mode) {
  ...
  return [
    ...
    'Concrete anti-pattern: WRONG = task 1 "inspect files/symbols" as read_only plus task 2 "implement feature". RIGHT = one modifying task whose inspectionHints and inspectionSummary carry the useful repository knowledge already discovered during planning. A read_only task inside a modifying request is appropriate only when the user explicitly requested that read-only result as an independent deliverable.',
    ...
    'When planning inspected concrete implementation details for a modifying task, put the reusable factual findings in inspectionSummary (bounded and concise): relevant existing functions/classes, current conventions, important field/key sets, existing test layout, and other facts that can save Worker A from re-discovering the same information. Do not paste raw tool transcripts.',
  ].join(' ');
}
...
function workerFlowInstructions(mode) {
  ...
  return [
    ...
    'Coordinator inspectionSummary is reusable planning evidence: use its concrete facts directly unless the exact text is needed to edit safely or a fact is uncertain. inspectionHints are locators for existing observed surfaces, not a checklist requiring every file to be reopened.',
