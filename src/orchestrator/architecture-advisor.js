    ...
    parameters: {
      ...
      properties: {
        ...
        patterns: {
          ...
          maxItems: 5,
          items: { type: 'string' },
          description: 'Only patterns that solve a concrete structural problem in this task. Empty is preferred when none are needed.',
        },
...
async function createArchitectureAdvisor(factory, taskId, route, risk) {
  ...
  const desiredEffort = routePolicy(route, risk, 'high').efforts.architect ?? 'medium';
...
async function runArchitectureAssessment(engine, factory, task, routing) {
  ...
  engine.ui?.agentConfiguration?.([
    { role: 'Software architect', model: advisor.model.name ?? advisor.model.id, effort: advisor.reasoningEffort },
  ]);
